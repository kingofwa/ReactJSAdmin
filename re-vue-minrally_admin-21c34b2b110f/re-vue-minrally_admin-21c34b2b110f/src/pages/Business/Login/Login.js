...
const BusinessLoginPage = () => {
  ...
  const onFinish = async values => {
    try {
      ...
      if (credentials?.role === USER_ROLE.BUSINESS) {
        dispatch(login(credentials));
        history.push(PATHS.business.dashboard);
      } else if (credentials?.role === USER_ROLE.SUPER_ADMIN) {
        dispatch(login(credentials));
  ...
  const goForgotPassword = () => {
    history.push(PATHS.business.forgotPassword);
  };

  return (
    <div className="login-wrapper">
      <h1 className="login-title">みんラリ事業者管理画面</h1>
      ...
      <div className="login_forgot-password" onClick={goForgotPassword}>
        ID/パスワードをお忘れの場合
      </div>
    </div>
  );
