...
import SpotManagementTable from './elements/SpotManagementTable';
import EventAnalysis from './elements/EventAnalysis';
...
const EventDetail = ({ location }) => {
  ...
  const onMatchedRoutes = () => {
    return [
      ...
      {
        route: {
          path: `${PATHS.businessDetail}?id=${get(eventDetail, [
            'owner_info',
            'id'
          ])}`,
          breadcrumbName: get(eventDetail, ['owner_info', 'business_name'])
        }
      },
      {
        route: {
  ...
  const fetchEventDetail = async () => {
    try {
      ...
      //   const photos = await srcToFile(response?.top_photo_url);
      //   setTopPhoto([photos]);
      // }
      ...
      if (response?.start_date && response?.end_date) {
        ...
        eventInfo.periodSetting = 'yes';
      } else {
        eventInfo.periodSetting = 'no';
      }
  ...
  const fetchSeries = async businessId => {
    try {
        ...
        <div className="event-detail-content">
          <Spin spinning={loading} className="custom-spinner">
            ...
            <SpotManagementTable
              ...
              isPublished={eventDetail?.status === 'published'}
            />
            <EventAnalysis />
          </Spin>
        </div>
