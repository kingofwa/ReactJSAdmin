...
import { getRallySpot, deleteSpot, updateSpotOrder } from '@api/event';
...
const EventRegistrationSpot = ({ location }) => {
  ...
  const onMatchedRoutes = () => {
    return [
      ...
      {
        route: {
          path: `${PATHS.businessDetail}?id=${eventInfo?.operator?.id}`,
          breadcrumbName: eventInfo?.operator?.name
        }
      },
      {
        route: {
          path: `${PATHS.eventRegistrationSpot}?id=${eventInfo?.game?.id}`,
  ...
  const onNextStep = () => {
    ...
    if (size(spots) > 2 && isAllSpotRegistered) {
      const url = `${PATHS.eventRegistrationSystemSetting}?rallyId=${rallyId}`;
      history.push(url);
    } else {
  ...
  const onCreateSpot = () => {
    history.push(`${PATHS.eventRegistrationSpotSetting}?rallyId=${rallyId}`);
  };
  ...
  const onSaveDraft = () => {
    ...
    const url = `${PATHS.businessDetail}?id=${eventInfo?.operator?.id}`;
    history.push(url);
  };

  const onUpdateSpotOrder = async () => {
    ...
    return (
      <div className="spot-item">
        ...
        <CustomButton
          ...
          onClick={() => {
            history.push(
              `${PATHS.eventRegistrationSpotSetting}?rallyId=${rallyId}&id=${spot?.id}`
            );
          }}
        >
          編集
...
export default EventRegistrationSpot;
