...
const SeriesDetail = ({ location }) => {
  ...
  const onMatchedRoutes = () => {
    return [
      ...
      {
        route: {
          path: `${PATHS.businessDetail}?id=${get(seriesDetail, [
            'owner_info',
            'id'
          ])}`,
          breadcrumbName: get(seriesDetail, ['owner_info', 'business_name'])
        }
      },
      {
        route: {
  ...
  const fetchSeriesDetail = async () => {
    try {
      ...
      setBusinessId(response?.extra?.operator?.id);
  ...
  const onSubmit = async values => {
    try {
      ...
      const startDate = moment(values?.period?.start_date)
        .tz('Asia/Tokyo')
        .format(DATE_DEFAULT);
      const endDate = moment(values?.period?.end_date)
        .tz('Asia/Tokyo')
        .format(DATE_DEFAULT);
      const params = {
        name: values?.name,
    ...
    if (loading) {
      return (
        <div className="custom-spin">
          <Spin spinning className="custom-spinner" />
        </div>
      );
    }
    return (
