...
import { getUserPlayingGames } from '@api/user';
...
const PlayerGameTable = () => {
  ...
  const fetchData = async (page = 1) => {
    try {
      setLoading(true);
      const response = await getUserPlayingGames(id, { page, per: pageSize });
  ...
  const eventColumns = [
    ...
    },
    {
      ...
      // render: fastest_finish_time =>
      //   fastest_finish_time
      //     ? `${fastest_finish_time.days}:${fastest_finish_time.hours}:${fastest_finish_time.minutes}:${fastest_finish_time.seconds}`
      //     : '-'
