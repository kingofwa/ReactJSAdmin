...
const ProfilePlayerGrand = ({ id }) => {
  ...
  const handleFavoriteSeri = (seriId, type) => {
    if (auth.isLoggedIn) {
      showLoadingAnim();
  ...
  const handleUnfavoriteSeri = (seriId, type) => {
    if (auth.isLoggedIn) {
      showLoadingAnim();
      ...
      >
        {isSelf || profile?.setting?.show_participating_rally ? (
          <ListRally
            ...
            type="playing"
            handleFavorite={handleFavoriteSeri}
            typeList={TYPE_LIST.serie}
            handleUnfavorite={handleUnfavoriteSeri}
          />
        ) : (
...
export default ProfilePlayerGrand;
